...
public class RestController {
  ...
  private boolean hasError(Response response, byte[] responseBody) throws IOException {
    if (response.isSuccessful()) {
      ...
      return !StringUtils.isBlank(model.getReturnedMessage()) && (!response.request().url().encodedPathSegments().contains("createsessionjson") && model.getReturnedMessage().equals("Approved"));
    } else {
  ...
  private Model formatResponse(byte[] responseBody) throws IOException {
    return ((responseBody[0] == '[')) ? mapper.readValue(responseBody, Model[].class)[0] : mapper.readValue(responseBody, Model.class);
  }
